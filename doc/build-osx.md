...
This guide describes how to build reliquiaed, command-line utilities, and GUI on macOS
...
It is not necessary to build wallet functionality to run `reliquiaed` or  `elements-qt`.
...
Elements Core should now be available at `./src/reliquiaed`.
If you compiled support for the GUI, it should be available at `./src/qt/elements-qt`.

The first time you run `reliquiaed` or `elements-qt`, it will start downloading the blockchain.
...
```shell
./src/reliquiaed -daemon      # Starts the elements daemon.
