...
const std::string CBaseChainParams::RELIQUIAE = "reliquiae";
const std::string CBaseChainParams::RELIQUIAE1TEST = "reliquiaeest";
const std::string CBaseChainParams::RELIQUIAETESTNET = "reliquiaetestnet";


const std::string CBaseChainParams::DEFAULT = CBaseChainParams::RELIQUIAE;

void SetupChainParamsBaseOptions(ArgsManager& argsman)
{
    argsman.AddArg("-chain=<chain>", "Use the chain <chain> (default: liquidv1). Reserved values: main, test, signet, regtest, reliquiae, reliquiaeest", ArgsManager::ALLOW_ANY, OptionsCategory::CHAINPARAMS);
...
std::unique_ptr<CBaseChainParams> CreateBaseChainParams(const std::string& chain)
{
    ...
    } else if (chain == CBaseChainParams::RELIQUIAE) {
        return std::make_unique<CBaseChainParams>("reliquiae", 7041, 8332, 37041);
    } else if (chain == CBaseChainParams::RELIQUIAETEST) {
        return std::make_unique<CBaseChainParams>("reliquiaetest", 7040, 18332, 37040);  // Use same ports as customparams
    } else if (chain == CBaseChainParams::RELIQUIAETESTNET) {
