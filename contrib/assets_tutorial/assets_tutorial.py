...
parser.add_argument('--reliquiaed-dir', type=str, default='./src')
...
e1 = Daemon(
    "Reliquiae1",
    "elements",
    args.reliquiaed_dir + "/reliquiaed",
    "contrib/assets_tutorial/Reliquiae1.conf",
    not args.no_cleanup,
)
...
#     is no bitcoind. When validatepegin is set, reliquiaed attempts to connect to
...
# reliquiaed config, and run it without a Bitcoin node, but this means that you
...
try:
    e1.getinfo()
    print ("ERROR: was able to start an reliquiaed without a working bitcoind")
    sys.exit(1)
except:
    pass

# 1b. Start bitcoind, then reliquiaed. Initially, the bitcoind may be warming up and
#     inaccessible over RPC. reliquiaed can detect this case and will stall until the
...
new_issue = e1.issueasset(0, 1, False) # `False` tells reliquiaed not to blind the issuance
